...
class CT_Ultimate_GDPR_Service_bbPress extends CT_Ultimate_GDPR_Service_Abstract {
	...
	public function get_target_user_id() {
		...
		$user_email = CT_Ultimate_GDPR::instance()->get_admin_controller()->get_option_value(
	...
	public function bbpress_validation_filter( $args ) {
		...
		if ( ! empty( $_POST['action'] ) && in_array( $_POST['action'], $filter_actions ) ) {
			...
			$inject = CT_Ultimate_GDPR::instance()->get_admin_controller()->get_option_value('services_bbpress_consent_field', false, CT_Ultimate_GDPR_Controller_Services::ID);
