					...
					<?php foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) : ?>
						<td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							<?php elseif ( 'order-total' === $column_id ) : ?>
								...
								printf( _n( '%1$s (%2$s)', '%1$s (%2$s)', $item_count, 'woocommerce' ), $order->get_formatted_order_total(), $item_count );
								?>
							...
							<?php elseif ( 'order-actions' === $column_id ) : ?>
								...
								if ( ! empty( $actions ) ) {
									foreach ( $actions as $key => $action ) {
										if ($key == "invoice") {
										echo '<a href="' . esc_url( $action['url'] ) . '" class="woocommerce-button button ' . sanitize_html_class( $key ) . '">' . __('Preuzmite račun', 'kimnatura') .'</a>';
