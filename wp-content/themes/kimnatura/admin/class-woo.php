...
class Woo
{
    ...
    public function myprefix_cart_extra_info()
    {
        global $woocommerce;
        echo '<div style="display: none;" class="cart-extra-info">';
    ...
    public function wpo_wcpdf_thank_you_link($text, $order)
    {
        ...
        $text .= '<div class="thanks__button"><a class="checkout-button button alt wc-forward btn btn--primary-color" href="' . esc_attr($pdf_url) . '">' . __('Preuzmite PDF', 'kimnatura') . '</a></div>';
        //}
        return $text;
    }
     ...
     * @since 2.0.0
     */
    public function woocommerce_cart_is_empty_message()
    {
    ...
    public function multi_step()
    {
        ...
        $t .= '<a href="' . get_permalink(wc_get_page_id('payment')) . '">';
    ...
    public function shipping_method_notice()
    {
        ...
        foreach ($zone->get_shipping_methods(true) as $k => $method) {
            ...
            if ($method->id == 'free_shipping' && !empty($min_amount) && $cart_total < $min_amount && $zone->get_zone_name() == 'Croatia') {
                ...
                $message = sprintf(__('Dodajte proizvoda za još %s kako biste ostvarili besplatnu dostavu!', 'kimnatura'), wc_price($remaining));
            }
        }
                        ...
                        if ($method->is_enabled()) {
                            ...
                            if ($method->id == 'free_shipping') {
                                ...
                                if (!empty($min_amount) && $cart_total < $min_amount && $zone->get_zone_name() == 'Croatia') {
                                    $remaining = $min_amount - $cart_total;
                                    $message = sprintf(__('Ako dodate još %s ostvariti ćete besplatnu dostavu!', 'kimnatura'), wc_price($remaining));
                                }
                            }
                        }
                        //echo $value->id;
 ...
 */
    ...
    public function b4b_woocommerce_cart_item_name($product_name, $cart_item = "", $cart_item_key = "")
    {
     ...
     * @since 1.0.0
     */
    public function catalog_item_image($size = 'shop-catalog')
    {
    ...
    public function move_variations_single_price()
    {
        global $product, $post;
    ...
    public function get_variation_single_price($variable_product, $variation_id)
    {
        // Main Price
    ...
    public function replace_variation_single_price()
    {
        global $product;
 ...
 */
    ...
    public function bbloomer_payment_gateway_disable_country($available_gateways)
    {
        global $woocommerce;
        if (is_admin()) {
            return;
     ...
     * @return string
     */
    public function woocommerce_form_field($key, $args, $value = null)
    {
        $defaults = array(
            'type' => 'text',
            'label' => '',
