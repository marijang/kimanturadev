...
wp core install --url={kimnatura.test} --title={THEMENAME} --admin_user={ADMINUSER} --admin_email={ADMINMAIL}
...
```sh
wpcs wp-content/themes/kimnatura
```

Autofix theme for minor violations:

```sh
wpcbf wp-content/themes/kimnatura
```
